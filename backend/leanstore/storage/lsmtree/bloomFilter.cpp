...
unsigned intlog2(uint64_t x) {
   ...
   void BloomFilter::init(uint64_t n) {
      ...
      pagesBits = 1+(intlog2(sizeBytes/ btreePageSize));
   ...
   void BloomFilter::insert(uint64_t h) {
      ...
      while (!currentPage->isLeaf) {
         uint64_t numberOfEntriesBelowNodePerChild = 1; //if root node has between 2...512 children, these children are payload nodes, so per child one entry page
